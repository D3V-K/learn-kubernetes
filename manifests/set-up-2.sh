...
# kind prefixes the kubeconfig context it creates with "kind-".
KIND_CONTEXT="kind-kindcluster-box-2"

echo "[1/4] Creating kind cluster..."
kind create cluster --config ~/manifests/kind-cluster/kindconfig-2.yaml

echo "[2/4] Waiting for nodes to be Ready..."
kubectl wait --for=condition=Ready nodes --all --timeout=180s

echo "[3/4] Installing MetalLB ${METALLB_VERSION}..."
kubectl apply -f "https://raw.githubusercontent.com/metallb/metallb/${METALLB_VERSION}/config/manifests/metallb-native.yaml" --context "${KIND_CONTEXT}"

echo "Waiting for MetalLB pods to become ready..."
kubectl -n metallb-system rollout status deployment/controller --timeout=180s --context "${KIND_CONTEXT}"
kubectl -n metallb-system rollout status daemonset/speaker --timeout=180s --context "${KIND_CONTEXT}"
...
sed -e "s/POOL_START/${POOL_START}/" -e "s/POOL_END/${POOL_END}/" \
  ~/manifests/kind-cluster/metallb-config.yaml | kubectl apply -f - --context "${KIND_CONTEXT}"

echo "Done. Cluster nodes:"
kubectl get nodes -o wide --context "${KIND_CONTEXT}"
echo "MetalLB status:"
kubectl get pods -n metallb-system --context "${KIND_CONTEXT}"
